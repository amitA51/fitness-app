...
import { MODULAR_EXERCISE_GROUPS } from './exercises';
...
>[] = [
  // ==================== Warmup & Cardio ====================
  {
    ...
    defaultRestTime: 30,
    defaultSets: 2,
    notes: 'חימום לכל הגוף - מעלה דופק ומכין את המפרקים לאימון.',
    tutorialText: 'עמדו זקוף, קפצו לפיסוק תוך הרמת הידיים מעל הראש וחזרו. שמרו על קצב רצוף.',
  ...
  },
  {
    ...
    defaultRestTime: 0,
    defaultSets: 1,
    notes: 'קרדיו במאמץ אחיד - בונה סיבולת לב-ריאה.',
    tutorialText: 'התחילו בהליכה מהירה לחימום, ואז שמרו על קצב קבוע ונשימה רגועה.',
 ...
 *
 * The catalog is the original single-file list plus the modular groups under
 * `data/exercises/` (upper/lower/core strength additions, power, conditioning,
 * mobility). New material goes in a module rather than this file so each domain
 * stays reviewable and a missing area is visible from the directory listing.
 */
export const getBUILT_IN_EXERCISES = (now: string): Omit<PersonalExercise, 'id' | 'createdAt'>[] =>
  [...STATIC_BUILT_IN_EXERCISES, ...MODULAR_EXERCISE_GROUPS].map((ex) => ({
    ...ex,
    lastUsed: now,
    useCount: 0,
  }));
