...
import type { WorkoutDerivedValue, WorkoutState } from './workoutTypes';

export const calculateWorkoutDerivedValues = (
  state: WorkoutState
): WorkoutDerivedValue & { duration: number } => {
  ...
  // Current Set
  const currentSet =
    currentExercise?.sets?.[effectiveSetIndex] || createWorkoutSet({ reps: 0, weight: 0 }); // fallback
