...
import { CheckCircleIcon, ClockIcon, FlameIcon, TrophyIcon } from '../../icons';
...
export const AnimatedCounter: React.FC<AnimatedCounterProps> = memo(
  ({ value, suffix = '', duration = 1200, className = '' }) => {
...
export const ActivityRing: React.FC<ActivityRingProps> = memo(
  ({ progress, color, size = 80, strokeWidth = 8, delay = 0 }) => {
...
export const StatCard: React.FC<StatCardProps> = memo(
  ({ icon, label, value, suffix, color, delay = 0, ringProgress }) => {
    return (
      <motion.div
