...
import { useAudioBeep, useVoiceCountdown } from './useVoiceCountdown';
...
export function useWorkoutSettings(): UseWorkoutSettingsReturn {
  ...
  const get = useCallback(
    <K extends keyof WorkoutSettings>(key: K): WorkoutSettings[K] => {
  ...
  const updateSetting = useCallback(
    <K extends keyof WorkoutSettings>(key: K, value: WorkoutSettings[K]) => {
      dispatch({
  ...
  const keepScreenAwake = useCallback(() => {
    ...
    const requestWakeLock = async () => {
      try {
        ...
        logger.workout.warn('Wake lock failed', err);
      }
    };
...
export function useThemeSettings() {
  ...
  return {
    ...
    setTheme: (themeId: string) => updateSetting('selectedTheme', themeId as WorkoutTheme),
...
export function useRestTimerSettings() {
  const { get, updateSetting, announceCountdown, announceReady, playRestEndSound } =
    useWorkoutSettings();
...
export function useSmartRestSettings() {
  ...
  const getRestTimeForExercise = useCallback(
    (exerciseType: 'isolation' | 'compound' | 'heavy') => {
...
export function useWorkoutFlowSettings() {
  ...
  return {
    ...
    setShowRestBetweenExercises: (enabled: boolean) =>
      updateSetting('showRestBetweenExercises', enabled),
  };
}
...
export function useQuickActionsSettings() {
  ...
  return {
    ...
    setWeightButtonsEnabled: (enabled: boolean) =>
      updateSetting('enableQuickWeightButtons', enabled),
...
export function useTimerDisplaySettings() {
  ...
  return {
    ...
    setDisplayMode: (mode: 'countdown' | 'countup' | 'both') =>
      updateSetting('timerDisplayMode', mode),
 ...
 */
export function useWorkoutSettingValue<K extends keyof WorkoutSettings>(
  key: K
): WorkoutSettings[K] {
  const state = useWorkoutState();
  const settings = state.appSettings?.workoutSettings || {};
