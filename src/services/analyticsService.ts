...
function computeSessionStats(session: WorkoutSession): { volume: number; sets: number; reps: number } {
  let volume = 0;
  let sets = 0;
  let reps = 0;
  for (const exercise of session.exercises) {
...
export const calculateWeeklyVolumes = (
  ...
  for (let i = 0; i < sortedWeeks.length; i++) {
    const [weekKey, weekSessions] = sortedWeeks[i];
    ...
    for (const session of weekSessions) {
      ...
      for (const exercise of session.exercises) {
        ...
        byExercise[exercise.exerciseName] = (byExercise[exercise.exerciseName] || 0) + exerciseVolume;
      }
    }
    ...
    if (i > 0) {
      const prevVolume = result[i - 1].totalVolume;
      if (prevVolume > 0) {
...
export const calculateMuscleBalance = (
  ...
  const averageVolume = totalCurrentVolume > 0 && currentVolumes.size > 0
  ...
  for (const muscle of allMuscles) {
    ...
    const percentage = totalCurrentVolume > 0
      ? Math.round((current / totalCurrentVolume) * 100)
      : 0;
...
export const forecastProgress = (
  ...
  if (dataPoints.length < 2) {
    const lastActual = dataPoints.length > 0 ? dataPoints[dataPoints.length - 1].actual : 0;
    return {
...
export const getLastWorkoutSummary = (sessions: WorkoutSession[]): LastWorkoutSummary | null => {
  ...
  const last = completed.sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime())[0];
...
export const getMuscleGroupDaysSince = (sessions: WorkoutSession[]): MuscleGroupLastTrained[] => {
  ...
  return Array.from(muscleLastDate.entries()).map(([muscle, date]) => ({
    muscle,
    lastDate: date,
    daysSince: Math.floor((new Date(today).getTime() - new Date(date).getTime()) / (1000 * 60 * 60 * 24)),
  }));
};
...
export const getWeekOverWeekProgress = (sessions: WorkoutSession[]): ProgressDelta[] => {
  ...
  const thisWeek = sessions.filter(s => s.status === 'completed' && s.date >= thisWeekStart.toISOString().split('T')[0]);
  const lastWeek = sessions.filter(s => s.status === 'completed' && s.date >= lastWeekStart.toISOString().split('T')[0] && s.date < thisWeekStart.toISOString().split('T')[0]);

  const computeExerciseVolume = (list: WorkoutSession[]): Map<string, { name: string; volume: number }> => {
    const map = new Map<string, { name: string; volume: number }>();
    for (const session of list) {
      for (const exercise of session.exercises) {
        const vol = exercise.sets.filter(s => s.isCompleted && !s.isWarmup).reduce((sum, s) => sum + s.weight * s.reps, 0);
        const existing = map.get(exercise.exerciseId);
        map.set(exercise.exerciseId, { name: exercise.exerciseName, volume: (existing?.volume || 0) + vol });
      }
    }
    return map;
  };
  ...
  return Array.from(allExercises).map(id => {
    ...
    return {
      ...
      currentVolume,
      previousVolume,
      change: previousVolume > 0 ? Math.round(((currentVolume - previousVolume) / previousVolume) * 100) : 0,
    };
  });
};
  ...
  return sessions
    ...
    .reduce<StrengthProgressPoint[]>((points, session) => {
      for (const exercise of session.exercises) {
        ...
        const volume = exercise.sets.filter(s => s.isCompleted && !s.isWarmup).reduce((sum, s) => sum + s.weight * s.reps, 0);
