...
export const deleteBodyWeight = async (id: string): Promise<void> => {
    ...
    if (user) {
        syncWithRetry(
            () => deleteCloudBodyWeight(user.id, id),
            `deleteBodyWeight:${id}`
        );
    }
};
...
export const getPersonalExercises = async (): Promise<PersonalExercise[]> => {
    ...
    if (missingBuiltIns.length > 0) {
        const newExercises = missingBuiltIns.map((ex, index) => ({
            ...ex,
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            getCurrentUser().then(user => {
                if (user) {
                    syncWithRetry(
                        () => syncPersonalExercise(user.id, newExercise as any),
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            getCurrentUser().then(user => {
                if (user) {
                    syncWithRetry(
                        () => syncPersonalExercise(user.id, updated as any),
...
function getBuiltInExercises(now: string): Partial<PersonalExercise>[] {
    return [
        // ==================== Warmup & Cardio ====================
...
export const getBuiltInWorkoutTemplates = (): BuiltInWorkoutTemplate[] => [
    {
        ...
        exercises: [
            { name: 'סקוואט | Back Squat', muscleGroup: 'Legs', targetSets: 4, targetReps: 8, targetRestTime: 120 },
            { name: 'לחיצת חזה | Bench Press', muscleGroup: 'Chest', targetSets: 4, targetReps: 8, targetRestTime: 120 },
            { name: 'מתח | Pull Up', muscleGroup: 'Back', targetSets: 4, targetReps: 8, targetRestTime: 90 },
            { name: 'לחיצת כתפיים | Overhead Press', muscleGroup: 'Shoulders', targetSets: 3, targetReps: 10, targetRestTime: 90 },
            { name: 'כפיפת מוט | Barbell Curl', muscleGroup: 'Biceps', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'פשיטת מרפקים בכבל | Tricep Pushdown', muscleGroup: 'Triceps', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'פלאנק | Plank', muscleGroup: 'Core', targetSets: 3, targetReps: 60, targetRestTime: 45 },
        ],
    ...
    },
    {
        ...
        exercises: [
            { name: 'לחיצת חזה | Bench Press', muscleGroup: 'Chest', targetSets: 4, targetReps: 8, targetRestTime: 120 },
            { name: 'לחיצת חזה בשיפוע חיובי | Incline Dumbbell Press', muscleGroup: 'Chest', targetSets: 3, targetReps: 10, targetRestTime: 90 },
            { name: 'פרפר בכבלים | Cable Fly', muscleGroup: 'Chest', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'לחיצת כתפיים | Overhead Press', muscleGroup: 'Shoulders', targetSets: 4, targetReps: 8, targetRestTime: 90 },
            { name: 'הרחקה לצדדים | Dumbbell Lateral Raise', muscleGroup: 'Shoulders', targetSets: 3, targetReps: 15, targetRestTime: 60 },
            { name: 'פרפר הפוך במכונה | Reverse Pec Deck', muscleGroup: 'Shoulders', targetSets: 3, targetReps: 12, targetRestTime: 60 },
        ],
    ...
    },
    {
        ...
        exercises: [
            { name: 'מתח | Pull Up', muscleGroup: 'Back', targetSets: 4, targetReps: 8, targetRestTime: 120 },
            { name: 'משיכת פולי עליון | Lat Pulldown', muscleGroup: 'Back', targetSets: 3, targetReps: 10, targetRestTime: 90 },
            { name: 'חתירה במוט | Barbell Row', muscleGroup: 'Back', targetSets: 4, targetReps: 8, targetRestTime: 90 },
            { name: 'משיכה לפנים | Face Pull', muscleGroup: 'Back', targetSets: 3, targetReps: 15, targetRestTime: 60 },
            { name: 'פשיטת מרפקים בכבל | Tricep Pushdown', muscleGroup: 'Triceps', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'כפיפת מוט | Barbell Curl', muscleGroup: 'Biceps', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'כפיפת פטישים | Hammer Curls', muscleGroup: 'Biceps', targetSets: 3, targetReps: 12, targetRestTime: 60 },
        ],
    ...
    },
    {
        ...
        exercises: [
            { name: 'סקוואט | Back Squat', muscleGroup: 'Legs', targetSets: 4, targetReps: 8, targetRestTime: 120 },
            { name: 'דדליפט רומני | Romanian Deadlift (RDL)', muscleGroup: 'Legs', targetSets: 4, targetReps: 10, targetRestTime: 90 },
            { name: 'לחיצת רגליים | Leg Press', muscleGroup: 'Legs', targetSets: 3, targetReps: 12, targetRestTime: 90 },
            { name: 'פשיטת ברכיים | Leg Extension', muscleGroup: 'Legs', targetSets: 3, targetReps: 15, targetRestTime: 60 },
            { name: 'כפיפת ברכיים בשכיבה | Lying Leg Curl', muscleGroup: 'Legs', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'היפ ת\'ראסט | Hip Thrust', muscleGroup: 'Legs', targetSets: 3, targetReps: 12, targetRestTime: 90 },
            { name: 'הרמת עקבים | Calf Raise', muscleGroup: 'Legs', targetSets: 4, targetReps: 15, targetRestTime: 45 },
        ],
        muscleGroups: ['רגליים'],
        icon: '🦵',
    },
    {
        ...
        exercises: [
            { name: 'פלאנק | Plank', muscleGroup: 'Core', targetSets: 3, targetReps: 60, targetRestTime: 45 },
            { name: 'כפיפות בטן | Crunch', muscleGroup: 'Core', targetSets: 3, targetReps: 20, targetRestTime: 45 },
            { name: 'כפיפות בטן בכבל | Cable Crunch', muscleGroup: 'Core', targetSets: 3, targetReps: 15, targetRestTime: 60 },
            { name: 'הרמת רגליים בתלייה | Hanging Leg Raise', muscleGroup: 'Core', targetSets: 3, targetReps: 12, targetRestTime: 60 },
            { name: 'טוויסט רוסי | Russian Twist', muscleGroup: 'Core', targetSets: 3, targetReps: 20, targetRestTime: 45 },
            { name: 'פשיטת גב / סופרמן | Hyperextension', muscleGroup: 'Back', targetSets: 3, targetReps: 15, targetRestTime: 60 },
        ],
...
export const convertBuiltInToWorkoutTemplate = (builtin: BuiltInWorkoutTemplate): WorkoutTemplate => ({
    id: builtin.id,
    name: builtin.name,
