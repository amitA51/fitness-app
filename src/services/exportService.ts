import type { WorkoutSession, MealEntry, BodyWeightEntry } from '../types';
...
export async function generateWeeklyReport(): Promise<string> {
  ...
  const totalSets = weekSessions.reduce((sum, s) =>
    sum + (s.exercises || []).reduce((es, ex) =>
      es + (ex.sets || []).filter(set => set.isCompleted && !set.isWarmup).length, 0
    ), 0
  );
  ...
  const avgDuration = weekSessions.length > 0 ? Math.round(totalDuration / 60 / weekSessions.length) : 0;

  const exerciseDetails = weekSessions.map(s => {
    const date = new Date(s.startTime).toLocaleDateString('he-IL', { weekday: 'long', day: 'numeric', month: 'long' });
    const exercises = (s.exercises || []).map(e => {
      const vol = e.sets.filter(set => !set.isWarmup).reduce((sum, set) => sum + set.weight * set.reps, 0);
