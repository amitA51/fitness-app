...
export const deleteWorkoutTemplate = async (_userId: string, id: string): Promise<void> => {
  if (!isSupabaseConfigured() || !supabase) return;
...
export const deleteWorkoutSession = async (_userId: string, id: string): Promise<void> => {
  if (!isSupabaseConfigured() || !supabase) return;
...
export const deletePersonalExercise = async (_userId: string, id: string): Promise<void> => {
  if (!isSupabaseConfigured() || !supabase) return;

  const { error } = await supabase.from('personal_exercises').delete().eq('id', id);
...
export const syncBodyWeight = async (_userId: string, entry: BodyWeightEntry): Promise<void> => {
  if (!isSupabaseConfigured() || !supabase) return;
...
export const deleteBodyWeight = async (_userId: string, id: string): Promise<void> => {
  if (!isSupabaseConfigured() || !supabase) return;
...
export const initFirebase = async (): Promise<void> => {
  logger.sync.info('Firebase is deprecated - using Supabase instead');
};
