...
const syncWithRetry = (operation: () => Promise<void>, operationName: string): void => {
...
export const getThemePreference = (): string => {
  const settings: ReturnType<typeof loadSettings> = loadSettings();
  return (settings.workoutSettings as { selectedTheme?: string })?.selectedTheme || 'deepCosmos';
};
...
export const createWorkoutFromTemplate = async (
  templateId: string
): Promise<Omit<PersonalItem, 'id' | 'createdAt' | 'updatedAt'>> => {
